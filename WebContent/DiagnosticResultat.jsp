...
<body data-bs-spy="scroll" data-bs-target="#header-nav" tabindex="0">
    ...
    <section  class="d-flex align-items-center justify-content-center" style="margin-top: 50px;"></section>
            <div class="container position-relative" >
                <div class="row align-items-center pt-5">
                    <div class=" col-md-6 ps-md-5">
                        <img src="images/resultat.jpg" alt="image" class="img-fluid">
                    </div>
                    <div class="col-md-6 px-py-5" mar>
                        
                        <h1 class="fw-bold text-center">Resultat de votre test</h1>
                        <div class="col-md-5 offset-md-1">
                            <div  >
                                <label for="natureMenstruation" style="margin-top: 20px;">Le resultat de votre test est: ${risk}</label>
                                
                            </div>
                        
                           
     <form action="DownloadResultsServlet" method="post">
        <input type="hidden" name="risk" value="${risk}">
        <input type="hidden" name="agePuberte" value="${agePuberte}">
        <input type="hidden" name="debutMenstruation" value="${debutMenstruation}">
        <input type="hidden" name="douleurRapports" value="${douleurRapports}">
        <input type="hidden" name="aggravation" value="${aggravation}">
        <input type="hidden" name="accouchee" value="${accouchee}">
        <input type="hidden" name="antecedentsFamiliaux" value="${antecedentsFamiliaux}">
        <input type="hidden" name="dureeCycle" value="${dureeCycle}">
        <input type="hidden" name="difficulteConception" value="${difficulteConception}">
        <input type="hidden" name="poids" value="${poids}">
        <input type="hidden" name="taille" value="${taille}">
        <input type="hidden" name="bmi" value="${bmi}">
        <input type="hidden" name="intensiteDouleursAbdominales" value="${intensiteDouleursAbdominales}">
        <input type="hidden" name="periodeDouleursAbdominales" value="${periodeDouleursAbdominales}">
        <input type="hidden" name="dureeRegles" value="${dureeRegles}">
        <input type="hidden" name="natureMenstruation" value="${natureMenstruation}">
        <button class="btn btn-primary btn-lg mt-3" type="submit" class="btn btn-primary" style="background-color:#ff879a !important ">Telecharger le resultat</button>
        </form>
